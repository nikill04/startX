...
router.get('/startups', async (req, res) => {
  ...
  /*router.get: This listens for a GET request. GET requests are used when the frontend wants to read data without changing anything.

  Startup.find({}): Startup is the model you imported at the top. .find() is a MongoDB command.
    1. Inside the parentheses {} is the filter.
    2. An empty object {} means "no filter," so it returns everything in that collection.
    3. If you wrote Startup.find({ city: 'London' }), it would only return startups in London. 

  res.json(startups): This takes the array of documents returned by the database and converts it into JSON format to send back to the user.  */});
...
// Until Here if you have any doubts refer to the file backend/routers/admin/1_Dummy_AuthExplain.js 
...
router.get('/grant/:id', async (req, res) => {
  try {
    // 1. The code pauses here. No callback function needed.
    const GrantRequests = await GrantScheme.findById(req.params.id);

    // 2. Logic flows naturally
    if (!GrantRequests) {
...
router.post('/grant/selectreviewer/:requestId', async (req, res) => {
    try {
        const requestId = req.params.requestId;
        const selectedReviewerIds = req.body; // Array of currently selected reviewer IDs
        const grant = await GrantScheme.findById(requestId);
        if (!grant) {
            return res.status(404).json({ message: 'GrantScheme document not found' });
        }

        // Current reviewers in GrantScheme document
        const currentReviewerIds = grant.reviews.map(review => review.reviewer_id.toString());
        ...
        await Promise.all(reviewersToRemove.map(async reviewerId => {
            ...
            grant.reviews = grant.reviews.filter(review => review.reviewer_id.toString() !== reviewerId);
        }));

        grant.grant_status.status = "Under Review"
        grant.grant_status.decision_date = Date.now();
        // Save the updated GrantScheme document
        await grant.save();

        res.status(200).json({ message: "Reviewers updated successfully", eir: grant });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: "An error occurred while updating reviewers", error: error.message });
    }
...
// If you have any doubts in the above one, refer to the file backend/routers/admin/2_Dummy_AuthExplain.js
    ...
    try {
      // Find the grant request by ID and update its status to "In Progress"
        const updatedGrant = await GrantScheme.findByIdAndUpdate(
            ...
            { new: true } // Return the current updated document, to the variable updatedGrant.
        );
...
// If you have any doubts in the above one, refer to the file backend/routers/admin/3_Dummy_AuthExplain.js
...
router.post('/eir/update-status', async (req, res) => {
  ...
  try {
    const updatedRequest = await EIR.findByIdAndUpdate(requestId, { $set: statusUpdate }, { new: true });
    if (!updatedRequest) {
      return res.status(404).json({ message: 'EIR request not found' });
    }
